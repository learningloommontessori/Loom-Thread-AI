...
async function fetchAndDisplayLessons(userId) {
    ...
    // UPDATED: Added is_favorite to selection
    let { data: lessons, error } = await supabase
        .from('AIGeneratedContent')
        .select('id, created_at, topic, language, age, is_favorite, content_json')
        .eq('user_id', userId)
    ...
    if (error) {
        ...
        if(grid) grid.innerHTML = `<p class="text-red-400 col-span-full">Error loading history.</p>`;
        return;
    }
...
function createLessonCard(lesson) {
    ...
    // FAVORITE LOGIC
    const isFav = lesson.is_favorite === true;
    const favIcon = isFav ? 'favorite' : 'favorite_border';
    const favClass = isFav ? 'text-red-500' : 'text-gray-400 hover:text-red-500';
        ...
        <div class="lesson-card bg-black/30 backdrop-blur-sm rounded-lg shadow-lg overflow-hidden flex flex-col justify-between border border-transparent hover:border-purple-500 transition-all duration-300" data-lesson-id="${lesson.id}" data-topic="${lesson.topic}" data-age="${ageGroup}">
            <div class="p-6">
                <div class="flex justify-between items-start mb-3">
                    <h3 class="text-xl font-bold text-white line-clamp-2 pr-2">${lesson.topic}</h3>
                    <button class="fav-btn ${favClass} transition-colors focus:outline-none" title="Toggle Favorite" data-lesson-id="${lesson.id}" data-is-fav="${isFav}">
...
async function handleToggleFavorite(e) {
    ...
    if (error) {
        ...
        // Revert on error
        btn.dataset.isFav = isCurrentlyFav;
        icon.textContent = isCurrentlyFav ? 'favorite' : 'favorite_border';
        alert('Could not update favorite status.');
    }
}

// --- FILTER LOGIC UPDATED FOR FAVORITES ---
function filterLessons(searchTerm, ageFilter) {
    const term = searchTerm.toLowerCase();
    const selectedAge = ageFilter; // 'all', 'favorites', or 'Nursery', etc.
    ...
    cards.forEach(card => {
        ...
        // Check favorite status from the button in the DOM
        ...
        if (selectedAge === 'favorites') {
            ...
            isFilterMatch = cardAge === selectedAge;
        }
...
// --- EXISTING FUNCTIONS (Download, Share, Delete, Modal) ---
// (These remain largely the same, included for completeness)

async function handleDownloadLesson(lessonId) {
    if (!window.jspdf) return alert("PDF Library loading... please wait.");
    const { data: lesson, error } = await supabase.from('AIGeneratedContent').select('*').eq('id', lessonId).single();
    ...
    doc.setLineWidth(0.5); doc.setDrawColor(200, 200, 200); doc.line(margin, y, pageWidth - margin, y); y += 10;
    ...
    if (content) {
        for (const category in content) {
            ...
            addText(categoryTitle, 14, 'bold', [50, 0, 100]); y += 2;
            ...
            for (const sub in subcategories) {
                ...
                if (Array.isArray(body)) {
                    body.forEach(item => { addText(`• ${item}`, 11, 'normal', [50, 50, 50]); y -= 2; });
                    y += 4;
                } else {
...
async function openShareSelectionModal(event) {
    ...
    const { data: lessonData, error } = await supabase.from('AIGeneratedContent').select('*').eq('id', lessonId).single();
    if (error || !lessonData) { modalContent.innerHTML = `<p class="text-red-400 text-center text-sm">Error loading data.</p>`; return; }

    const options = generateShareableItems(lessonData);
    if (options.length === 0) { modalContent.innerHTML = `<p class="text-gray-400 text-center text-sm">No content found.</p>`; return; }

    let html = `<div class="flex flex-col h-full"><p class="text-xs text-gray-400 mb-2 px-1">Select threads to share.</p><div class="flex-grow overflow-y-auto pr-1 space-y-2 max-h-[55vh] custom-scrollbar">`;
    let currentGroup = '';
    options.forEach((opt, index) => {
        if (opt.group !== currentGroup) {
            currentGroup = opt.group;
            html += `<div class="sticky top-0 bg-gray-800/95 backdrop-blur z-10 py-1 px-1 border-b border-gray-700 mb-1 mt-2 first:mt-0"><h5 class="text-[10px] font-bold text-purple-400 uppercase tracking-wider">${currentGroup}</h5></div>`;
        }
        html += `<label class="flex items-center p-1.5 rounded-md bg-gray-700/20 border border-gray-700/50 hover:bg-gray-700/50 hover:border-purple-500/30 cursor-pointer transition-all group"><div class="flex items-center h-full"><input type="checkbox" class="share-checkbox form-checkbox h-3.5 w-3.5 text-purple-600 rounded border-gray-500 bg-gray-800 focus:ring-purple-500 focus:ring-1" value="${index}"></div><div class="ml-2.5 flex-1 min-w-0"><div class="flex items-center gap-1.5"><span class="material-symbols-outlined text-sm ${opt.iconColor}">${opt.icon}</span><h4 class="text-xs font-medium text-gray-200 group-hover:text-white truncate">${opt.label}</h4></div><p class="text-[10px] text-gray-500 line-clamp-1 leading-tight">${opt.preview}</p></div></label>`;
    });
    html += `</div><div class="pt-3 mt-2 border-t border-gray-700 flex justify-between items-center bg-gray-800 z-20"><button id="select-all-btn" class="text-[10px] text-purple-400 hover:text-purple-300 font-medium px-2 uppercase tracking-wide">Select All</button><button id="confirm-share-btn" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-1.5 px-4 rounded text-xs transition-all flex items-center shadow-md disabled:opacity-50"><span class="material-symbols-outlined mr-1.5 text-sm">groups</span> Share Bundle</button></div></div>`;
    ...
    confirmBtn.onclick = async () => {
        ...
        if (selectedIndices.length === 0) { alert("Select at least one item."); return; }
...
function generateShareableItems(lesson) {
    ...
    if (json.newActivities) Object.entries(json.newActivities).forEach(([key, val]) => { const title = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()); addTextItem("Classroom Activities", title, "Activity", val, "extension", "text-blue-400"); });
    if (json.movementAndMusic) Object.entries(json.movementAndMusic).forEach(([key, val]) => { const title = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()); addTextItem("Movement & Music", title, "Movement", val, "directions_run", "text-green-400"); });
    if (json.socialAndEmotionalLearning) Object.entries(json.socialAndEmotionalLearning).forEach(([key, val]) => { const title = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()); addTextItem("Social & Emotional", title, "SEL", val, "diversity_3", "text-orange-400"); });
    if (json.teacherResources) Object.entries(json.teacherResources).forEach(([key, val]) => { const title = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()); addTextItem("Teacher Guide", title, "Resource", val, "menu_book", "text-teal-400"); });
    return items;
}
...
async function executeBatchShare(items, lessonData, buttonElement) {
    ...
    const combinedContent = items.map(item => `
        <div class="shared-item mb-6">
            <h4 class="text-purple-300 font-bold text-lg mb-2 flex items-center"><span class="material-symbols-outlined text-base mr-2">${item.icon}</span> ${item.label}</h4>
            <div class="text-gray-300 leading-relaxed">${item.content}</div>
        </div><hr class="border-gray-700 my-4 last:hidden">`).join('');
    ...
    if (error) {
        ...
        setTimeout(() => { document.getElementById('lesson-modal').classList.add('hidden'); }, 1500);
    }
}
...
async function handleViewLesson(event) {
    ...
    if (error || !data) { modalContent.innerHTML = `<p class="text-red-400 text-center">Could not load lesson details.</p>`; return; }
